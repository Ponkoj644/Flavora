...
const MenuPage = () => {
    ...
    return (
        <section className="w-full min-h-screen flex flex-col items-center py-20 bg-[var(--theme-bg)] transition-colors duration-500">
            <h1 className="text-4xl md:text-5xl font-extrabold mt-10 mb-5 tracking-tight text-center" style={{ color: "var(--theme-primary)" }}>
                Menu
            </h1>
            ...
            {/* Menu items */}
            <div className=" w-full md:max-w-6xl flex flex-col md:flex-row gap-4 items-start justify-between">
                <div className="w-full  lg:w-1/2 mx-2 text-start">
                    {filteredItems.length === 0 ? (
                        ...
                        filteredItems.map((item) => (
                            ...
                            >
                                <div
                                    className="flex items-start gap-6 py-4 border-b border-[var(--theme-border)] hover:bg-[var(--theme-border)]/20 transition"
                                >
                                    ...
                                    <div className="flex-1 ">
                                        <h2 className="text-lg sm:xl md:text-2xl font-bold mb-2" style={{ color: "var(--theme-primary)" }}>
                                            {item.title}
                                        </h2>
                                        ...
                                        <div className="flex items-center justify-between gap-2">
                                            ...
                                            <div className="text-lg md:text-xl lg:text-2xl font-bold text-start mx-2 min-w-[70px]" style={{ color: "var(--theme-primary)" }}>
                                                {item.price}
                                            </div>
                                        </div>
                                    </div>
                ...
                {/* Best Choices */}
                <div className="w-full lg:w-1/2 mx-2 text-start">
                    <h2 className="text-xl font-bold mb-4" style={{ color: "var(--theme-primary)" }}>Best Choices</h2>
                    ...
                    <div className="flex flex-col gap-4 mx-2 ">
                        {popularItems.map(item => (
                            <Link
                                key={item.slug}
                                href={`/menu/${item.slug}`}
                                className="flex items-center gap-3  rounded-lg hover:bg-[var(--theme-border)]/30 transition"
                                style={{ textDecoration: "none" }}
                            >
